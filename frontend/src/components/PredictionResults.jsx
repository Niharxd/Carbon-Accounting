...
export default function PredictionResults() {
  ...
  useEffect(() => {
    function handle(e) {
      ...
      if (detail.result) { setResult(detail.result); setShowAllRecs(false); }
  ...
  if (!result) {
    return (
      <div className="rounded-2xl border border-dashed border-slate-700 bg-slate-950/60 p-10 flex flex-col items-center justify-center gap-5 min-h-[200px] sm:min-h-[320px] h-full text-center">
  ...
  const visibleRecs = showAllRecs ? recs : recs.slice(0, 1);
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl p-4 bg-slate-900/80 border border-slate-700/50">
            ...
            <p className={`text-4xl font-black ${
              result.anomaly_detected ? 'text-red-400'
              : result.predicted_emissions > 18.8 ? 'text-amber-300'
              : 'text-emerald-300'
          ...
          <div className="rounded-2xl p-4 bg-slate-900/80 border border-slate-700/50 space-y-3">
            <div className="flex items-center justify-between">
              <p className="text-slate-400 uppercase tracking-widest text-[11px] font-semibold">Recommendations</p>
              {recs.length > 1 && (
                <button
                  onClick={() => setShowAllRecs((v) => !v)}
                  className="text-xs text-emerald-400 hover:text-emerald-300 font-semibold transition"
                >
                  {showAllRecs ? 'Show less ↑' : `+${recs.length - 1} more`}
                </button>
              )}
            </div>
            {visibleRecs.map((rec, i) => (
              <div key={i} className="flex items-start gap-2">
                ...
                <div>
                  ...
                  {rec.impact && (
                    <span className="inline-block mt-1.5 text-xs text-emerald-400 bg-emerald-500/10 border border-emerald-500/30 rounded-full px-2 py-0.5">
                      {rec.impact} impact
                    </span>
                  )}
                </div>
              ...
              <div className="space-y-2">
                ...
                <div className="mt-2 h-1.5 rounded-full bg-slate-700 overflow-hidden">
                  <div
                    ...
                    style={{ width: `${Math.max(2, simulation.reduction_pct)}%` }}
                  />
                </div>
