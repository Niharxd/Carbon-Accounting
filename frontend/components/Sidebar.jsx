...
const navItems = [
  { name: 'Dashboard', href: '/', icon: '📊', color: 'from-blue-400 to-blue-600', bgColor: 'bg-blue-500/20' },
  { name: 'Predictions', href: '#predictions', icon: '⚡', color: 'from-emerald-400 to-emerald-600', bgColor: 'bg-emerald-500/20' },
  { name: 'Analytics', href: '#analytics', icon: '📈', color: 'from-purple-400 to-purple-600', bgColor: 'bg-purple-500/20' },
  { name: 'History', href: '#history', icon: '📋', color: 'from-orange-400 to-orange-600', bgColor: 'bg-orange-500/20' },
  { name: 'Profile', href: '#profile', icon: '👤', color: 'from-pink-400 to-pink-600', bgColor: 'bg-pink-500/20' },
];
  ...
  return (
    <>
      ...
      <aside className={`
        ...
        glass border-r border-emerald-500/30
        flex flex-col
      ...
      `}>
        ...
        <nav className="flex-1 px-3 py-6 space-y-2 overflow-y-auto">
          {navItems.map((item, idx) => (
            ...
            >
              ...
              <div className={`absolute inset-0 bg-gradient-to-r ${item.color} opacity-0 group-hover:opacity-15 transition-opacity duration-300 rounded-xl`}></div>
              
              {/* Border glow on hover */}
              <div className="absolute inset-0 border border-transparent group-hover:border-emerald-500/40 rounded-xl transition-colors duration-300"></div>
              ...
              {/* Right accent line */}
              <div className={`absolute right-0 top-0 bottom-0 w-1 bg-gradient-to-b ${item.color} scale-y-0 group-hover:scale-y-100 transition-transform duration-300 origin-top`}></div>
            </Link>
          ))}
        </nav>
        ...
        <div className="px-3 py-4 border-t border-emerald-500/20">
          <div className="bg-gradient-to-r from-emerald-600/40 to-teal-600/30 rounded-xl p-4 border border-emerald-500/40 space-y-2">
            <div className="flex items-center gap-2">
              <span className="flex w-2.5 h-2.5 rounded-full bg-emerald-400 animate-pulse"></span>
