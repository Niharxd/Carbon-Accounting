    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 h-16 transition-all duration-300 flex items-center px-4 gap-4 ${isScrolled ? 'glass shadow-2xl shadow-emerald-500/10 border-b border-emerald-500/30' : 'border-b border-emerald-500/20'}`}>
      ...
      <div className="flex items-center gap-3">
        {username ? (
          ...
          <>
            <Link href="/login" className="px-3 py-1.5 text-xs text-slate-300 hover:text-emerald-300 transition-colors font-semibold">
              Login
            </Link>
            <Link href="/signup" className="px-4 py-1.5 text-xs gradient-primary hover:shadow-lg hover:shadow-emerald-500/50 text-white rounded-lg transition-all font-bold">
              Sign Up
            </Link>
          </>
        )}
